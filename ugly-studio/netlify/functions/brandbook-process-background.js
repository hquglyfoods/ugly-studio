...
const SECTIONS = [
  ...
  { key: "audience", tokens: 1200,
    spec: '{"audience": {"target": string, "market_position": string, "edge": string, "not_for": string}}',
    ask: "Capture who the brand is for (age, lifestyle, psychographics), where it sits relative to fast food, fast casual and premium dining, what it can claim that competitors cannot, and who it is explicitly not for." },
  ...
  { key: "visual", tokens: 2600,
    spec: '{"logo": {"marks": [{"name": string, "use": string, "file": string}], "clear_space": string, "never": [string]}, "illustration": {"style": string, "items": [{"name": string, "use": string, "never": string}]}}',
    ask: "Capture every logo mark, where it is used, and which file formats are needed for it, the clear space rule, and every logo misuse rule. Then the illustration style and each illustration with where it is used and where it must never be used." },
